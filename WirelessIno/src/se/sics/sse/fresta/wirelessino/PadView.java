...
public class PadView extends SurfaceView implements Callback, Runnable {
    ...
    public Rect screen, bar1, bar2, notif, acc, platoon;
    ...
    public void onDraw(Canvas canvas) {
        try {
            ...
            canvas.drawRect(acc, pBlue);
            canvas.drawRect(platoon, pYellow);
...
//				}
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        bar2 = new Rect(3 * left,
                h - w / 2,
                5 * left,
                h + w / 2);

        balls[0] = new Ball(left - w,
                top - 5 - w + (bottom - top) / 2,
                right + w,
                top + 5 + w + (bottom - top) / 2,
                Ball.LEFT_BAR);

        balls[1] = new Ball(4 * left - w - (w / 2),
                top - w - 5 + (bottom - top) / 2,
                4 * left + w + (w / 2),
                top + w + 5 + (bottom - top) / 2,
                Ball.RIGHT_BAR);
        ...
        notif = new Rect(screen.centerX() - w - (UMBRAL_TACTIL / 2),
                (screen.height() / 8) - w - 5,
                (screen.centerX()) + w + (UMBRAL_TACTIL / 2),
                (screen.height() / 8) + w + 5);

        acc = new Rect(screen.centerX() + screen.width() / 16,
                top + screen.height() / 8,
                screen.centerX() + screen.width() / 4,
                top + screen.height() / 16);

        platoon = new Rect(screen.centerX() + screen.width() / 16,
                top - screen.height() / 8,
                screen.centerX() + screen.width() / 4,
                top - screen.height() / 16);

        bluinoBMP = resizeImage(this.getContext(),
                R.drawable.bluinotooth,
                7 * w,
                2 * getHeight() / 8);
